    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        register_button.setOnClickListener(View.OnClickListener {
            ...
            if(username == "" || email == "" || password1 == "" || password2 == ""){
                popAlert.showAlert("Sorry!", "All the fields should be filled properly", false, null)
            }
            else if(password1.length < 5){
                popAlert.showAlert("Sorry!", "Password must have at least 6 characters", false, null)
            }
            else if(password1 != password2){
                popAlert.showAlert("Sorry!", "Password confirmation failed", false, null)
            } else if (!Application().validateEmail(email)){
                popAlert.showAlert("Sorry!", "Email not valid", false, null)
            } else {
                auth.createUserWithEmailAndPassword(email, password1)
                    .addOnCompleteListener(this) { task ->
                        if (task.isSuccessful) {
                            ...
                            popAlert.showAlert("Congrats!", "Registration successful", true, intent)
                        } else {
                            // If sign in fails, display a message to the user.
                            Log.w(TAG, "createUserWithEmail:failure", task.exception)
                            popAlert.showAlert("Sorry!", "Authentication failed", false, null)
                        }
                    }
            }
        })
